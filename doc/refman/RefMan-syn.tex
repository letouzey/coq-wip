...
\section{Numeral Notations
\comindex{Numeral Notation}}

Starting with version 8.7, the following command allows to customize
the way numeral literals are parsed and printed:

\begin{quote}
{\tt Numeral Notation $ty$ $pa$ $pr$~:~{\scope}.}
\end{quote}
...
functions.
The
parsing function $pa$ should have one of the following types:
\begin{itemize}
\item {\tt Decimal.int -> $ty$}
\item {\tt Decimal.int -> $ty$ option}
\item {\tt Decimal.uint -> $ty$}
\item {\tt Decimal.uint -> $ty$ option}
\item {\tt Z -> $ty$}
\item {\tt Z -> $ty$ option}
\end{itemize}
And the printing function $pr$ should have one of the following types:
\begin{itemize}
\item {\tt $ty$ -> Decimal.int}
\item {\tt $ty$ -> Decimal.int option}
\item {\tt $ty$ -> Decimal.uint}
\item {\tt $ty$ -> Decimal.uint option}
\item {\tt $ty$ -> Z}
\item {\tt $ty$ -> Z option}
\end{itemize}
...
\begin{Variants}
\item
{\tt Numeral Notation $ty$ $pa$ $pr$~:~{\scope} (warning after $n$).}

When a literal larger than $n$ is parsed, a warning message about
possible Stack Overflow will be displayed.

\item
{\tt Numeral Notation $ty$ $pa$ $pr$~:~{\scope} (abstract after $n$).}

When a literal $m$ larger than $n$ is parsed, the result will be
{\tt ($pa$~$m$)}, without reduction of this application to a normal form.
Here $m$ will be a {\tt Decimal.int} or {\tt Decimal.uint} or {\tt Z},
depending on the type of the parsing function $pa$. This allows a more
compact representation of literals in types such as {\tt nat}, and
