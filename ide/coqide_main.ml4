...
(* NB: Since [win32_interrupt] involves some hack about the process console,
...
IFDEF QUARTZ THEN
  let () =
    GtkosxApplication.Application.set_menu_bar osx#as_osxapplication
      (GtkMenu.MenuShell.cast
	 (Coqide_ui.ui_m#get_widget "/CoqIde MenuBar")#as_widget)
  in
  let () =
    GtkosxApplication.Application.insert_app_menu_item osx#as_osxapplication
      (Coqide_ui.ui_m#get_widget "/CoqIde MenuBar/Edit/Prefs")#as_widget 1
  in
  let () =
    GtkosxApplication.Application.set_help_menu osx#as_osxapplication
