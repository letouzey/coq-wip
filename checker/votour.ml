...
let visit_vo f =
  ...
  let _ = input_binary_int ch in (* magic number *)
  let md = (System.marshal_in f ch : library_disk) in
  let _ = System.marshal_in f ch in (* digest *)
  let table = (System.marshal_in f ch : Safe_typing.LightenLibrary.table) in
  let () = close_in ch in
  let md' =
    { md with md_compiled =
        Obj.magic (Safe_typing.LightenLibrary.load table md.md_compiled) }
  in
  let () = CObj.register_shared_size md' in
  let () = init () in
  visit Values.v_lib (Obj.repr md') []
