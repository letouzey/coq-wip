...
module Make (T : TY) = struct
  type idx = int
  type t = T.t
  let pool = (Hashtbl.create 7 : (idx, t) Hashtbl.t)
  let put obj =
    let idx = Hashtbl.length pool in
    Hashtbl.add pool idx obj;
    idx
  let get idx =
    try Hashtbl.find pool idx with Not_found -> assert false
end
