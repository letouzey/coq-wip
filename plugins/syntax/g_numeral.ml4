...
let rec constr_of_glob_constr vl = function
  | {CAst.v = Glob_term.GRef (gr, gllo)} ->
      constr_of_global_reference gr
  | {CAst.v = Glob_term.GVar (id)} ->
...
let rec glob_constr_of_constr ?loc c = match Constr.kind c with
  | Var id ->
      CAst.make ?loc (Glob_term.GRef (VarRef id, None))
  | App (c, ca) ->
  ...
  | Construct (c, _) ->
      CAst.make ?loc (Glob_term.GRef (ConstructRef c, None))
  | Const (c, _) ->
      CAst.make ?loc (Glob_term.GRef (ConstRef c, None))
  | Ind (ind, _) ->
      CAst.make ?loc (Glob_term.GRef (IndRef ind, None))
  | x ->
...
let vernac_numeral_notation ty f g sc waft =
  ...
  let crq = CAst.make (CRef (Qualid lqid, None)) in
  let identref s = (None, Names.Id.of_string s) in
  let app x y = CAst.make (CApp ((None, x), [(y, None)])) in
  let cref s = CAst.make (CRef (Ident (identref s), None)) in
  let arrow x y =
    CAst.make
      ...
      CAst.make
        (CCast
           (CAst.make (CRef (f, None)),
            CastConv
              (arrow (cref "Z") (app (cref "option") crq))))
    in
  ...
  | IndRef (sp, spi) ->
      ...
      let _ =
        ...
        let c =
          CAst.make
            (CCast
               (CAst.make (CRef (g, None)),
                CastConv
                  (arrow crq (app (cref "option") (cref "Z")))))
        in
        let (sigma, env) = Lemmas.get_current_context () in
