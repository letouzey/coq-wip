...
let rec scalar n = function
 ...
 | Omult(t1,t2) ->
     Err.error "Omega: Can't solve a goal with non-linear products"
...
let shrink_pair f1 f2 =
  begin match f1,f2 with
   ...
   | t1,t2 ->
       ...
       flush Pervasives.stdout; Err.error "shrink.1"
  end
...
let reduce_factor = function
  ...
  | Omult(Oatom v,c) ->
	...
	| _ -> Err.error "condense.1" in
	[Lazy.force coq_c_reduce], Omult(Oatom v,Oint(compute c))
  | t -> Err.error "reduce_factor.1"
...
let total_reflexive_omega_tactic gl =
  ...
  with NO_CONTRADICTION -> Err.error "ROmega can't solve this system"
