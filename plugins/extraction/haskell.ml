...
let pp_global h k r =
  ...
  else str (Common.pp_global h k r)

(*s Pretty-printing of types. [par] is a boolean indicating whether parentheses
    are needed or not. *)

let rec pp_type h par vl t =
  let rec pp_rec par = function
    ...
    | Tglob (IndRef(kn,0),l)
         when not (keep_singleton ())
              && MutInd.equal kn (mk_ind "Coq.Init.Specif" "sig")
         -> pp_type h true vl (List.hd l)
    | Tglob (r,l) ->
          pp_par par
            (pp_global h Type r ++ spc () ++
             prlist_with_sep spc (pp_type h true vl) l)
    | Tarr (t1,t2) ->
	pp_par par
  ...
  function
    ...
    | MLcons (_,r,a) as c ->
	  ...
	  | _ when is_native_char c -> pp_native_char c
          | [] -> pp_global h Cons r
	  | [a] ->
            pp_par par (pp_global h Cons r ++ spc () ++ pp_expr h true env [] a)
	  | _ ->
            pp_par par (pp_global h Cons r ++ spc () ++
                        prlist_with_sep spc (pp_expr h true env []) a)
	end
    | MLtuple l ->
        assert (List.is_empty args);
    ...
    | MLcase (typ,t,pv) ->
        apply2
          (v 0 (str "case " ++ pp_expr h false env [] t ++ str " of {" ++
                fnl () ++ pp_pat h env pv))
    | MLfix (i,ids,defs) ->
    ...
    | MLmagic a ->
        pp_apply (str "unsafeCoerce") par (pp_expr h true env [] a :: args)
    | MLaxiom -> pp_par par (str "Prelude.error \"AXIOM TO BE REALIZED\"")

and pp_cons_pat h par r ppl =
  pp_par par
    (pp_global h Cons r ++ space_if (not (List.is_empty ppl)) ++
     prlist_with_sep spc identity ppl)

and pp_gen_pat h par ids env = function
  | Pcons (r,l) -> pp_cons_pat h par r (List.map (pp_gen_pat h true ids env) l)
  | Pusual r -> pp_cons_pat h par r (List.map Id.print ids)
  | Ptuple l -> pp_boxed_tuple (pp_gen_pat h false ids env) l
  | Pwild -> str "_"
...
and pp_one_pat h env (ids,p,t) =
  ...
  hov 2 (str " " ++
         pp_gen_pat h false (List.rev ids') env' p ++
         str " ->" ++ spc () ++
  ...
  let pp_constructor (r,l) =
    (pp_global h Cons r ++
     match l with
       ...
       | _  -> (str " " ++
                prlist_with_sep
                  (fun () -> (str " ")) (pp_type h true pl) l))
  in
  str (if Array.is_empty cv then "type " else "data ") ++
  pp_global h Type (IndRef ip) ++
...
let rec pp_ind h first kn i ind =
  if i >= Array.length ind.ind_packets then
    if first then mt () else fnl ()
  else
    let ip = (kn,i) in
    let p = ind.ind_packets.(i) in
    if is_custom (IndRef (kn,i)) then pp_ind h first kn (i+1) ind
    else if p.ip_logical then
      pp_logical_ind p ++ pp_ind h first kn (i+1) ind
    else
      pp_one_ind h ip p.ip_vars p.ip_types ++ fnl () ++
      pp_ind h false kn (i+1) ind

(*s Pretty-printing of a declaration. *)

let pp_decl h = function
  | Dind (kn,i) when i.ind_kind == Singleton ->
     let h,def = pp_singleton h kn i.ind_packets.(0) in
     h, def ++ fnl ()
  | Dind (kn,i) ->
     let h = store_ind h kn i in
     h, hov 0 (pp_ind h true kn 0 i)
  | Dtype (r, l, t) ->
	...
	in
        ...
        h, hov 2 (str "type " ++ pp_global h Type r ++ spc () ++ st) ++ fnl2 ()
  | Dfix (rv, defs, typs) ->
	...
	(fun i r ->
	  ...
	  if void then mt ()
            ...
            hov 2 (names.(i) ++ str " :: " ++ pp_type h false [] typs.(i)) ++
            fnl () ++
	    (if is_custom r then
		(names.(i) ++ str " = " ++ str (find_custom r))
	     else
                (pp_function h (empty_env ()) names.(i) defs.(i)))
	    ++ fnl2 ())
	rv
  | Dterm (r, a, t) ->
      if is_inline_custom r then h, mt ()
        ...
        let e = pp_global h Term r in
        h,
        hov 2 (e ++ str " :: " ++ pp_type h false [] t) ++ fnl () ++
	  if is_custom r then
	    hov 0 (e ++ str " = " ++ str (find_custom r) ++ fnl2 ())
	  else
            hov 0 (pp_function h (empty_env ()) e a ++ fnl2 ())
